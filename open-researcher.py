...
async def call_groq_async(session, messages, model=DEFAULT_MODEL):
    """
    Asynchronously call the groq chat completion API with the provided messages.
        ...
        async with session.post(groq_URL, headers=headers, json=payload) as resp:
            if resp.status == 200:
                ...
                try:
                    ...
                    print("Unexpected groq response structure:", result)
                    return None
            else:
                text = await resp.text()
                print(f"groq API error: {resp.status} - {text}")
                return None
...
async def generate_search_queries_async(session, user_query):
    ...
    response = await call_groq_async(session, messages)
    if response:
        try:
...
async def fetch_webpage_text_async(session, url):
    """
    Asynchronously retrieve the text content of a webpage using crawl4ai.
    """
    browser_config = BrowserConfig()
    run_config = CrawlerRunConfig()
...
async def get_new_search_queries_async(
    session, user_query, previous_search_queries, all_contexts
):
    """
...
async def generate_final_report_async(session, user_query, all_contexts):
    ...
    report = await call_groq_async(session, messages)
    return report
...
async def process_link(session, link, user_query, search_query):
    ...
    if usefulness == "Yes":
        context = await extract_relevant_context_async(
            session, user_query, search_query, page_text
        )
        if context:
...
async def async_main():
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        while iteration < iteration_limit:
            ...
            # For each search query, perform DuckDuckGo searches concurrently.
            search_tasks = [
                perform_search_async(session, query) for query in new_search_queries
            ]
        ...
        final_report = await generate_final_report_async(
            session, user_query, aggregated_contexts
        )
        print("\n==== FINAL REPORT ====\n")
        print(final_report)
...
# it runs everything on a loop and and rechecks it and has pointers which if it doesnt get then it redos the same thing eh so each function tries to get exact response more like hardcoded sorta
